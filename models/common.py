...
class SAConv2d(ConvAWS2d):
    ...
    (dilation d and 3d) on weight-standardized kernels, with pre/post context
    aggregation. Reference: Wang et al., "Rethinking Receptive Fields for
    Small Object Detection" (2022).
    """

    default_act = nn.SiLU()

    def __init__(self, in_channels, out_channels, kernel_size=3, s=1, p=None, g=1, d=1, act=True, bias=False):
        super().__init__(in_channels, out_channels, kernel_size, stride=s, padding=autopad(kernel_size, p, d),
    ...
    def forward(self, x):
        ...
        x = x + avg_x
        avg_x = F.pad(x, pad=(2, 2, 2, 2), mode='replicate')
        ...
        ori_p = self.padding
        ori_d = self.dilation
        self.padding = tuple(3 * p_ for p_ in self.padding)
...
class BottleneckSAC(nn.Module):
    """Bottleneck whose second conv is a SAConv2d (drop-in for Bottleneck at SAC positions)."""
...
class C3SAC(C3):
    """C3 module with BottleneckSAC blocks (inner bottlenecks use e=1.0 to preserve channel width)."""
